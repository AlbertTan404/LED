...
class Sampler(nn.Module):
    ...
    def forward(
        ...
        if global_decoding_params.method:
            ...
            # Save original logits for logprob computation before LED modifies them
            if return_logprob:
                # LED operates on multi-layer logits (B, d, V), use the final layer (index 0) for logprobs
                orig_logits_for_logprob = logits_output.next_token_logits[:, 0, :].clone()

            if global_decoding_params.method == 'led':
                batch_next_token_ids = latent_exploration_decoding(
                ...
                batch_next_token_ids = dola_decoding(
                    logits_output=logits_output,
                    decoding_params=global_decoding_params,
            ...
            # Compute logprobs for the selected tokens (needed for verl training)
            if return_logprob:
                if not sampling_info.is_all_greedy:
                    orig_logits_for_logprob.div_(sampling_info.temperatures)
                logprobs = torch.nn.functional.log_softmax(orig_logits_for_logprob, dim=-1)
