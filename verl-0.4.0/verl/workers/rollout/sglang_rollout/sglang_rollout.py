...
class SGLangRollout(BaseRollout):
    ...
    def _build_led_kwargs(self):
        """Build LED (Latent Exploration Decoding) kwargs from config if available."""
        led_config = self.config.get("led", None)
        if led_config is None:
            return {}

        from led.decoding import LatentDecodingParams

        method = led_config.get("method", None)
        if not method:
            return {}

        decoding_params = LatentDecodingParams(
            method=method.lower(),
            think_only=led_config.get("think_only", 1),
            d=led_config.get("d", 8),
            k=led_config.get("k", 8),
            ln=led_config.get("ln", 0),
            do_exploit=led_config.get("do_exploit", 1),
        )

        extra_kwargs = {
            "decoding_params": decoding_params,
            "enable_think_only": bool(led_config.get("think_only", 1)),
            "disable_cuda_graph": True,
            "disable_overlap_schedule": True,
        }
        logger.info(f"LED decoding enabled: method={method}, params={decoding_params}")
        return extra_kwargs
    ...
    def _init_inference_engine(self, trust_remote_code, actor_module, port):
        ...
        # Build LED kwargs if configured
        led_kwargs = self._build_led_kwargs()
